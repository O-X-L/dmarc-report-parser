...
def get_ip_address_country(ip_address, db_path=None):
    ...
    if db_path is None:
        with pkg_resources.path(resource_dbip, "dbip-country-lite.mmdb") as path:
            db_path = path
...
def get_service_from_reverse_dns_base_domain(base_domain,
    ...
    # todo: replace with working
    # if url is None:
    #     url = ("https://raw.githubusercontent.com/domainaware"
    #            "/dmarc_report_parser/master/dmarc_report_parser/"
    ...
    if len(reverse_dns_map) == 0:
        ...
        with pkg_resources.path(resource_maps, "base_reverse_dns_map.csv") as path:
